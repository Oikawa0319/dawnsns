...
<div class='follows_icon_lists'>
  <!-- タイトルテキスト表示 -->
  <h1 class="follows_list_text">Follower List</h1>
  <!-- アイコンリスト範囲指定/開始 -->
  ...
  <div class="right_post">
    ...
    <div class="right_top">
      ...
      <div class="username list_username">
        <!-- ログインしてるユーザーではないユーザーの名前を表示/followsコントローラのfollowerメソッドからusernameを引継いで表示 -->
        <p>{{ $user->username }}</p><br>
      </div>
      <!-- 投稿日範囲指定/開始 -->
      <div class="date">
        <!-- 投稿日テキスト表示/followsコントローラのfollowerメソッドからcreated_atを引継いで表示 -->
        <p>{{ $user->created_at }}</p><br>
      </div>
    </div>
    <!-- posts範囲指定/開始 -->
    <div class="posts">
      <!-- postテキスト表示/followsコントローラのfollowerメソッドからpostsを引継いで表示 -->
      <p>{{ $user->posts }}</p><br>
    </div>
